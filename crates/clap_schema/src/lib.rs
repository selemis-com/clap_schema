...
//!
//! Every contract-visible operation has an ordinary Rust type deriving [`Operation`]. Its canonical
//! `#[clap_schema::handler]` supplies the hidden handler contract; `clap_schema` provides the
//! `Operation` capability when both declarations are present. The handler's declared `Result<T, E>`
//! remains the sole source of its successful output contract. For non-unit `T`, the crate
...
//! let create = contract.command_for::<CreateArgs>().expect("create operation is registered");
...
//! ```
//!
//! `CreateArgs` is both the Clap payload and the Rust operation identity. Its [`Operation`] derive
//! marks that identity, while the hidden contract generated by `#[handler]` completes the
//! `Operation` capability; removing the handler or
//! attaching a second canonical handler therefore fails to compile. Derive-based executable
...
//!
//! let contract = Cli::schema()?;
//! let stash = contract.command_for::<StashArgs>().expect("stash operation is registered");
//! let list = contract.command_for::<ListArgs>().expect("list operation is registered");
...
//! # Handler forms
//!
//! `#[handler]` supports synchronous, `const fn`, and asynchronous free functions with exactly one
//! typed operation input. Receiver-based handlers, including handlers with additional runtime
//! context parameters, put `#[handler]` on a dedicated inherent impl block containing exactly one
//! receiver method. The enclosing `Self` type is then the operation identity, so shapes such as
//! `run(self, context, output_mode)` require no parameter guessing. Generic handlers and opaque
//! `impl Trait` return types are rejected because they do not
//! identify one concrete output contract.
//!
//! # Builder-style Clap
//!
//! Builder applications use the same handler-derived operation contracts. There is no API
...
//! assert!(contract.command_for::<CreateOperation>().and_then(|command| command.output).is_some());
//! # Ok::<(), clap_schema::Error>(())
//! ```
//!
...
//! assert_eq!(
//!     contract.extended_schema_for_operation::<ListArgs>().unwrap()["allOf"]
//!         .as_array()
//!         .map(Vec::len),
//!     Some(2),
//! );
...
//! use [`ContractBuilder::extend`] and [`ContractBuilder::operation_with_extension`].
//!
...
pub use clap_schema_derive::{CliSchema, CommandGroup, CommandSchema, Operation, handler};
...
/// when the root parser type is itself an [`Operation`]. Root derives may declare an
...
pub trait CliSchema: clap::CommandFactory {
    ...
    /// # Errors
    ///
    /// Returns an error when derived operation registration disagrees with the
...
/// one named tuple payload implementing [`Operation`]; its canonical `#[handler]` supplies the
/// successful-output contract required by that trait.
