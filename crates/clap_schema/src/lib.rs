...
//! successful output contract. For non-unit `T`, the crate requires
...
/// variants use one named tuple payload with a canonical `#[schema_handler(PayloadType)]` supplying
/// its successful-output contract.
