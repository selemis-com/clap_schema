...
//! in schema metadata. `subcommands` alone represents a group-only parent; add `handler` when
//! the payload also has its own `#[schema_handler(...)]`.
//!
...
///
/// Prefer `#[derive(CliSchema)]` for derive-based Clap applications. Add `#[schema(handler)]`
/// when the root parser also has its own `#[schema_handler(...)]`. Root derives may declare an
