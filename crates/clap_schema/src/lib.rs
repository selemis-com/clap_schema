...
//!
//! Every contract-visible operation has an ordinary Rust type implementing [`Operation`]. The
//! implementation is empty; its canonical `#[clap_schema::handler]` supplies the hidden handler
//! contract required by the trait. The handler's declared `Result<T, E>` remains the sole
...
//! ```
//!
//! `CreateArgs` is both the Clap payload and the Rust operation identity. Its empty [`Operation`]
//! implementation requires the hidden contract generated by `#[handler]`; removing the handler or
...
//! }
//!
//! impl clap_schema::Operation for StashArgs {}
//!
//! #[derive(Subcommand, CommandSchema)]
//! enum StashCommands {
//!     List(ListArgs),
